/* JetVaults accessibility widget for hasdera-elad.com
...
(function () {
  ...
  var KEY = 'hasdera-elad-a11y';
  var GOLD = '#B09754';
  var INK = '#2A1B16';

  var css = [
    '#jv-a11y{position:fixed;bottom:18px;right:18px;z-index:99999;font-family:"Assistant",Arial,sans-serif;direction:rtl}',
    '#jv-a11y-toggle{display:grid;place-items:center;width:52px;height:52px;border:none;border-radius:50%;background:' + GOLD + ';color:#fff;cursor:pointer;box-shadow:0 8px 24px rgba(42,27,22,.35);transition:filter .2s ease,box-shadow .2s ease}',
    '#jv-a11y-toggle:hover{filter:brightness(1.08);box-shadow:0 12px 30px rgba(42,27,22,.45)}',
    '#jv-a11y-toggle svg{width:30px;height:30px}',
    '#jv-a11y-panel{position:absolute;bottom:64px;right:0;width:304px;max-width:calc(100vw - 36px);background:#fff;border:1px solid rgba(176,151,84,.4);border-radius:12px;box-shadow:0 24px 60px rgba(42,27,22,.35);padding:18px;box-sizing:border-box}',
    '#jv-a11y-panel *{box-sizing:border-box}',
    '#jv-a11y-panel .jv-a11y-title{font-weight:700;color:' + INK + ';margin:0 0 12px;font-size:16px}',
    '#jv-a11y-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px}',
    '#jv-a11y-grid button{display:flex;flex-direction:column;align-items:center;gap:4px;font-family:inherit;font-size:13px;font-weight:500;color:' + INK + ';background:#F1F1F1;border:1px solid rgba(176,151,84,.4);border-radius:8px;padding:10px 6px;cursor:pointer;transition:background-color .15s ease,border-color .15s ease,color .15s ease}',
    ...
    '#jv-a11y-statement{display:block;margin-top:12px;text-align:center;font-size:14px;font-weight:700;color:' + INK + ';text-decoration:underline!important}',

    /* --- effect modes (classes on <html>) --- */
    /* text size: zoom keeps the px-based layout scaling correctly */
    ...
    'html[class*="jv-a11y-"] header,html[class*="jv-a11y-"] main,html[class*="jv-a11y-"] footer{filter:var(--jv-filter,none)}',

    /* light background */
    'html.jv-a11y-light body,html.jv-a11y-light header,html.jv-a11y-light main,html.jv-a11y-light main section,html.jv-a11y-light footer{background:#fff!important;background-image:none!important}',
    'html.jv-a11y-light h1,html.jv-a11y-light h2,html.jv-a11y-light h3,html.jv-a11y-light p,html.jv-a11y-light span,html.jv-a11y-light li,html.jv-a11y-light a,html.jv-a11y-light strong{color:#111!important}',
